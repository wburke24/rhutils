...
troubleshoot = F
if (troubleshoot) {
  check_worldfile("worldfiles/RithetCreek.world")
  # ================ FOR TROUBLESHOOTING ================
}


# ==================== TRIM, RESAMPLE, Fill, d8 direction, accumulation, Streams, Basin, basin outlet  ====================
maps_out = wbox_dem2streams_gauge_basin(source_dem = source_dem, source_gauge = source_gauge, res = res, stream_threshold = stream_threshold, gauge_snap_dist = gauge_snap_dist, output_dir = output_dir, plots = T, writeplots = T, overwrite = T)

# ==================== TRIM AGAIN TO BASIN ====================
maps_out = wbox_trim_to_basin_overwrite(maps = maps_out, basin = maps_out[["basin"]], output_dir = output_dir, plots = T, writeplots = T)
...
p_map = rast(file.path(output_dir, "basin.tif"))
names(p_map) = "patches"
...
# fix DEM
dem = rast(file.path(output_dir, "dem.tif"))
plot(dem)
demtrim = crop(dem, rast(file.path(output_dir, "basin.tif")))
plot(demtrim)

writeRaster(demtrim, filename = file.path(output_dir, "dem.tif"), overwrite=T)
