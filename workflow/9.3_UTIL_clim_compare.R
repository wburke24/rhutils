...
agg_clim$source = "basestation_gridmet_aggregated"
...
# plots ------
pr2 = climate %>% group_by(source, yd) %>% summarise(Precipitation = mean(rain)) %>%
  ggplot() +
  geom_line(aes(x = yd, y = Precipitation, color = source) ) +
  labs(title = "Cumulative Annual Precipitation", x = "Year Day" ) +
  theme(legend.title=element_blank())

tmin2 = climate %>% group_by(source, yd) %>% summarise(`Minimum Temperature` = mean(tmin)) %>%
  ggplot() +
  geom_line(aes(x = yd, y = `Minimum Temperature`, color = source) ) +
  labs(title = "Average Daily Minimum Temperature", x = "Year Day" ) +
  theme(legend.title=element_blank())

tmax2 = climate %>% group_by(source, yd) %>% summarise(`Maximum Temperature` = mean(tmax)) %>%
  ggplot() +
  geom_line(aes(x = yd, y = `Maximum Temperature`, color = source) ) +
  labs(title = "Average Daily Maximum Temperature", x = "Year Day" ) +
  theme(legend.title=element_blank())

grid2 = plot_grid(pr2, tmin2, tmax2, ncol=1)

ggsave("plots/clim_compare_yearday.jpg", grid2, width = 10, height = 12)
