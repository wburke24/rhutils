...
output_world_paths = list.files(file.path("worldfiles/",dest),pattern = ".world$",full.names = T)
output_flow_paths = list.files(file.path("flowtables/",dest),pattern = ".flow$",full.names = T)
rules = gsub(".world","", gsub("RedRockMSR30m_tree2shrubnobare_ruleID","", list.files(file.path("worldfiles/",dest),pattern = ".world$")))
IDs = rules
